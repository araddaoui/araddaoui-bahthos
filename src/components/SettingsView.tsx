  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#fafaf8] overflow-y-auto p-4 md:p-6" id="settings-view">
      <div className="max-w-3xl mx-auto w-full space-y-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-red-100 shadow-2xs space-y-5" id="settings-projects-management">
          <h2 className="text-xs font-bold text-red-800 flex items-center gap-2 border-b border-red-50 pb-2">
            <FolderGit2 className="w-4 h-4 text-[#0d6264]" />
            <span>إدارة وحذف المشاريع البحثية</span>
          </h2>

          {/* List of projects */}
          {projects && projects.length > 0 && (
            <div className="space-y-3">
              <span className="text-xs font-semibold text-gray-700 block">قائمة المشاريع المتاحة:</span>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <FolderGit2 className={`w-4 h-4 ${isActive ? "text-[#0d6264]" : "text-gray-400"}`} />
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-gray-800">{proj.name}</span>
                            {isActive && (
                              <span className="text-[10px] bg-[#0d6264] text-white px-2 py-0.5 rounded-md font-bold">
                                النشط حالياً
                              </span>
                            )}
                          </div>
                          {proj.dateCreated && (
                            <span className="text-[10px] text-gray-400 block mt-0.5">تاريخ الإنشاء: {proj.dateCreated}</span>
                          )}
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        {!isActive && onSwitchProject && (
                          <button
                            onClick={() => onSwitchProject(proj.id)}
                            className="px-3 py-1.5 bg-white text-teal-700 hover:bg-teal-50 border border-teal-200 rounded-lg text-xs font-bold transition-all cursor-pointer"
                          >
                            الانتقال للمشروع
                          </button>
                        )}
                        <button
                          ...
                          className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
                          title={`حذف مشروع ${proj.name}`}
                          id={`settings-delete-project-btn-${proj.id}`}
                        >
          ...
          <div className="border-t border-gray-100 pt-3 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              <span className="text-xs font-semibold text-gray-700 block">إعادة تعيين مساحة العمل بالمشروع الحالي</span>
              <p className="text-[11px] text-gray-400 leading-relaxed">
                سيؤدي هذا الإجراء إلى حذف جميع المستندات التي قمت برفعها في هذا المشروع، ومسح سجل الدردشة، وإعادة المنصة لحالتها الافتراضية الأولى.
              </p>
            </div>
            <button
              onClick={handleReset}
              className="px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 rounded-xl text-xs font-bold transition-all duration-200 flex items-center justify-center gap-1.5 self-start md:self-auto cursor-pointer"
            >
              <AlertTriangle className="w-4 h-4" />
              <span>مسح بيانات المشروع والبدء من جديد</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {projectToDelete && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4" id="delete-project-settings-modal">
          <div className="bg-white rounded-2xl border border-[#e2e2dd] shadow-xl max-w-md w-full p-6 text-right space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center gap-3 border-b border-gray-100 pb-3">
              ...
              <div>
                <h3 className="text-sm font-black text-red-800">حذف المشروع: {projectToDelete.name}</h3>
                <p className="text-[10px] text-gray-400 font-medium mt-0.5">إزالة كاملة للمستندات والدردشات</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                ...
                id="confirm-delete-project-settings-btn"
              >
                <Trash2 className="w-3.5 h-3.5" />

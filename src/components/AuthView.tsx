...
export default function AuthView({ onSuccess, onSkip, initialIsSignUp = false }: AuthViewProps) {
  ...
  const translateError = (errCode: string, errMessage?: string): string => {
    switch (errCode) {
      ...
      case "auth/unauthorized-domain":
        return "هذا النطاق غير معتمد لإجراء المصادقة. يرجى إضافة نطاق موقعك الحالي إلى قائمة Authorized Domains في إعدادات Firebase Authentication.";
      default:
        return `حدث خطأ: (${errCode || "غير معروف"}). ${errMessage || ""}`;
    }
  };
    ...
    try {
      if (isSignUp) {
        ...
        try {
          ...
          console.warn("Email verification sending notice:", verifyErr);
        }
        setVerificationNotice(
          `تم إنشاء حسابك بنجاح! أرسلنا رسالة تأكيد إلى بريدك الإلكتروني (${email.trim()}). يرجى مراجعة صندوق الوارد (أو مجلد الرسائل غير المرغوب فيها) لتأكيد حسابك.`
        );
      } else {
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      setError("يرجى إدخال البريد الإلكتروني المسجل لإرسال رابط الاستعادة.");
      return;
    }
    ...
    try {
      ...
      setSuccessMessage(
        "تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني بنجاح! تفقد صندوق الوارد للمتابعة."
      );
    } catch (err: any) {
  ...
  return (
    <div className="w-full bg-white text-[#1f1f1f] p-6 md:p-8 space-y-6 relative overflow-hidden font-sans antialiased" dir="rtl" id="auth-card-container">
      ...
      {/* Screen 1: Registration Email Verification Notice Screen */}
      {verificationNotice ? (
        <div className="space-y-6 text-center animate-fadeIn" id="auth-verification-screen">
          <div className="mx-auto bg-teal-50 text-[#094d4e] p-4 rounded-2xl w-16 h-16 flex items-center justify-center border border-teal-100 shadow-xs">
            <Mail className="w-8 h-8 text-[#094d4e]" />
          </div>
          <div className="space-y-2">
            <h2 className="text-xl font-bold text-[#094d4e]">تحقق من بريدك الإلكتروني</h2>
            <p className="text-xs text-gray-600 leading-relaxed max-w-sm mx-auto">
              {verificationNotice}
            </p>
          </div>
          <div className="p-4 bg-[#fcfbfa] border border-[#e2e2dd] rounded-xl text-xs text-gray-500 text-right space-y-1">
            <p className="font-bold text-gray-700 flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />
              <span>تم تفعيل الحساب في مساحة العمل</span>
            </p>
            <p>يمكنك البدء بالبحث والتحليل التوليفي فوراً، ويُفضل النقر على رابط التأكيد في بريدك لضمان أمان حسابك الدائم.</p>
          </div>
          <button
            type="button"
            onClick={onSuccess}
            className="w-full py-3 bg-[#094d4e] hover:bg-[#073d3e] text-white font-bold rounded-xl text-sm transition-all shadow-sm active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
            id="auth-verification-continue-btn"
          >
            <span>المتابعة إلى مساحة البحث</span>
            <ArrowRight className="w-4 h-4 rotate-180" />
          </button>
        </div>
      ) : isForgotPassword ? (
        /* Screen 2: Forgot Password Screen */
        <div className="space-y-6 animate-fadeIn" id="auth-forgot-password-screen">
          <div className="flex flex-col items-center text-center space-y-3">
            <div className="bg-[#094d4e] text-white p-3.5 rounded-2xl flex items-center justify-center shadow-md">
              <KeyRound className="w-7 h-7" />
            </div>
            <div className="space-y-1">
              <h1 className="text-2xl font-extrabold text-[#094d4e] tracking-tight">استعادة كلمة المرور</h1>
              <p className="text-xs text-gray-400 font-medium">bahthOS • نظام حسابات الباحثين</p>
            </div>
          ...
          {successMessage && (
            <div className="bg-emerald-50 text-emerald-800 text-xs font-semibold p-4 rounded-xl border border-emerald-200 flex items-center gap-2.5 animate-fadeIn">
          ...
          <div className="text-center pt-2">
            <button
              ...
              className="text-xs font-semibold text-[#094d4e] hover:underline cursor-pointer flex items-center justify-center gap-1 mx-auto"
        ...
        /* Screen 3: Main Login & Registration Screen */
        <>
          {/* Header and Branding */}
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {!isSignUp && (
                  <button
                    ...
                    className="text-[11px] font-semibold text-[#094d4e] hover:underline cursor-pointer"
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-hidden p-1 transition-colors cursor-pointer"

...
export async function parseDocumentFile(file: File): Promise<ParsedDocumentResult> {
  ...
  if (isPdf) {
    ...
    // Only when client-side text extraction failed (placeholder above) retain a
    // base64 copy for small files so the server can re-parse the raw PDF and, in
    // the last resort, hand it to Gemini multimodal. Larger files and successful
    // extractions skip base64 to keep the payload well under Vercel's 4.5MB cap.
    const pdfNeedsBase64 = extractedText.startsWith("[مستند PDF:") && file.size <= 3_000_000;
    const pdfBase64 = pdfNeedsBase64 ? await fileToBase64(file) : undefined;
  ...
  if (isDocx) {
    try {
      ...
      // Only when mammoth failed to extract a text layer retain a base64 copy
      // for the server-side parse path (same cap as PDFs).
      const docxNeedsBase64 = !docText && file.size <= 3_000_000;
      const docxBase64 = docxNeedsBase64 ? await fileToBase64(file) : undefined;
    ...
    const failedDocxBase64 = file.size <= 3_000_000 ? await fileToBase64(file) : undefined;

    return {
      text: `[مستند Word: ${fileName}]`,
      base64: failedDocxBase64,
      mimeType: file.type || "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      fileName
    };
  }

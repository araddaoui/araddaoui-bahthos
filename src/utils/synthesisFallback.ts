...
function extractDocSubstance(src: any, idx: number, safeTopic: string) {
  ...
  if (lowerTitle.includes("post human") || lowerTitle.includes("post-human")) {
    ...
    supportingEvidence = "تثبت الدراسة أن الاعتماد الكلي على الآلة يضعف التماسك الثقافي والأسلوبي، مؤكدة أن المترجم البشري يظل الركيزة الأساسية في إعادة التأويل والتكيف السياقي للمخرجات المعقدة.";
    ...
    supportingEvidence = "يسجل التقييم ارتفاعاً ملحوظاً في الأخطاء التركيبية والدلالية عند معالجة التراكيب المجازية والأدبية، مما يخفض درجة مفهومية النص بنسبة قابلة للقياس مقارنة بالنقل البشري.";
    ...
    supportingEvidence = "تظهر المعطيات التفاضلية تفوق الترجمة العصبية في النصوص التقريرية والإدارية، في حين تتراجع كفاءتها في النصوص التخصصية الدقيقة والقانونية مقارنة بالأنظمة المنهجية الأخرى.";
    ...
    supportingEvidence = "تثبت النتائج الميدانية تحسن السرعة والاتساق المصطلحي عند استخدام المعاجم المدمجة، مصحوباً بظهور تحديات تتصل بظاهرة العَمَى التحريري (Automation Bias) لدى المراجعين.";
    ...
    coreIssue = "تحليل الفاعلية والاستقلالية البشرية (Human Agency) في البيئات الرقمية والحضرية المؤتمتة";
    ...
    supportingEvidence = "تستعرض الدراسة تحولات الفاعلية البشرية في البيئات الرقمية (Posthuman agency in digitally mediated cities)، مؤكدة أن الوساطة التقنية لا تستبدل التكيف البشري، بل تتطلب استقلالية مرنة وآليات تحرير واعية لضمان جودة القرار الميداني.";
  ...
  } else if (rawSummary.length > 30 && !/[a-zA-Z]{8,}/.test(rawSummary)) {
    ...
    const variedGaps = [
      `الحاجة إلى قياس تتبعي ميداني يفحص استقرار المخرجات في بيئات تشغيلية مختلفة عبر فترات زمنية ممتدة.`,
      `محدودية التغطية الميدانية في عينات البحث الحالية، مما يستدعي إجراء دراسات مقارنة مسحية موسعة.`,
      `غياب المعايير القياسية الموحدة لتقييم الأثر الاقتصادي والعملياتي المباشر عند تطبيق النموذج على مجالات تخصصية دقيقة.`,
      `عدم اكتمال الأطر المعيارية المعنية بقياس التفاعل التكيفي بين العنصر البشري والأدوات المؤتمتة تحت ظروف العمل الميداني.`,
      `اقتصار الفحص على النطاق المحلي المحدد دون اختبار مرونة الخوارزميات والأنظمة المعتمدة أمام التغيرات السياقية المتسارعة.`
    ];
    ...
    const variedGaps = [
      `توسيع نطاق التغطية البحثية ليشمل بيئات تشغيلية متعددة ومتنوعة لضمان عمومية النتائج.`,
      `إجراء مراجعة ميدانية تتبعية تفحص تغير المتغيرات مع مرور الوقت وفي ظل المستجدات التقنية.`,
      `سد النقص في البيانات المعيارية المسحية المقارنة بين التطبيقات النظرية والممارسات الميدانية.`,

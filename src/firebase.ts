 ...
 * - If user is signed in with Firebase, provides `Bearer <idToken>`.
 * - If guest user, provides `Bearer guest-token` and `x-guest-mode: true` to preserve Guest mode.
 */
export async function getAuthHeaders(): Promise<Record<string, string>> {
  if (auth.currentUser) {
    try {
      ...
      console.warn("[Auth] Failed to acquire Firebase ID token:", e);
    }
  }
  return {

...
const synthesisSource = readFileSync("src/server/routers/synthesis.ts", "utf8");
assert.match(synthesisSource, /الإحاطة المقارنة/);
const apiSource = readFileSync("api/index.ts", "utf8");
assert.match(apiSource, /src\/server\/routers\/tts/);
assert.doesNotMatch(dalilCardSource, /falling back to WebSpeech/);
assert.doesNotMatch(dalilCardSource, /playChunkWithWebSpeechFallback\(rawSegments/);
assert.match(synthesisSource, /التقاطع والتأطير المشترك/);

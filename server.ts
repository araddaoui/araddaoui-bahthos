...
app.post("/api/analyze-document", async (req, res) => {
  ...
  try {
    const ai = getAiClient();
    const promptText = `أنت محرك متقدم وسريع للتحليل والتدقيق الأكاديمي في نظام "بحث OS" المخصص لمساعدة الباحثين.
قم بتحليل المستند المرفق بدقة مطلقة واستخرج منه ما يلي كـ JSON مطابق تماماً للمخطط المطلوب.
هام جداً للسرعة والأداء: صغ الملخص والتعاريف بأسلوب مكثف للغاية وموجز (سطرين إلى ثلاثة كحد أقصى للملخص، وجملة واحدة قصيرة للتعاريف):
...
3. ملخص أكاديمي بليغ ومكثف (summary): صغ ملخصاً أكاديمياً مكثفاً وموجزاً للغاية يلخص الأهداف والنتائج في فقرة واحدة قصيرة جداً (3 أسطر كحد أقصى).
4. مصطلحات أكاديمية أو تقنية (terms): استخرج حتى 3 من أبرز المصطلحات التقنية أو الأكاديمية أو العلمية الهامة الواردة في النص وطبق عليها عملية التحقق ثنائية الحقول (Two-Field Verification Process) والاختبار المستقل عن التخصص لضمان دقة التعريب وتصحيح أي تعريب صوتي.
${useMultimodalPdf ? "5. نص المستند المستخرج (extractedText): نظراً لقراءة الملف مباشرة كـ PDF، يرجى استخراج ونقل أول 1000 كلمة من النص الأصلي للمستند بدقة بالغة في حقل extractedText." : ""}`;
